...
public class ActionJoueur {
	...
	public int [] choixCase(Plateau plateauDuJeu, int[][] tableauAffichage, Boolean equipe, ile ileDuJeu){
		...
		do{
			do{
				coordonnees=this.getCoordonneesClic(plateauDuJeu,999,999);
	...
	public int [] choixCase(ile ileDuJeu, Plateau plateauDuJeu, int[][] tableauAffichage, int x, int y, Personnage perso){
			...
			for(int j=x-1;j<x+2;j++){
				if(perso instanceof Voleur || perso instanceof Guerrier){
					...
					if(perso.getDeplacement() && (tableauAffichage[i][j]==perso.getIdBateau() || tableauAffichage[i][j]==15))
						plateauDuJeu.setHighlight(j, i, Color.BLUE);
					if(perso.getAction() && tableauAffichage[i][j]>5 && tableauAffichage[i][j]<15 && ileDuJeu.getTableau()[j][i].getPersonnageCourant().getEquipe()==perso.getEquipe())
						plateauDuJeu.setHighlight(j, i, Color.BLUE);
				}else if(((i==(y-1) || i==(y+1)) && j==x) || ((j==(x-1) || j==(x+1)) && i==y)){
					if(perso.getAction() && (tableauAffichage[i][j]==1 || tableauAffichage[i][j]==4 || (tableauAffichage[i][j]<14 && tableauAffichage[i][j]>5 && ileDuJeu.getTableau()[j][i].getPersonnageCourant().getEquipe()==perso.getEquipe())))
						plateauDuJeu.setHighlight(j, i, Color.BLUE);
		...
		if(perso instanceof Explorateur){
			do{
				do{
					coordonnees=this.getCoordonneesClic(plateauDuJeu,x,y);
					xEvent=coordonnees[0];
					yEvent=coordonnees[1];
	 ...
	 */
	public int [] getCoordonneesClic(Plateau plateauDuJeu, int x, int y){
		int [] res= new int[2];
		int keyCode = 0;
		InputEvent event;
		...
		do{
			...
			if(plateauDuJeu.getId()==1)
				...
				keyCode=127;
		}while(!(event instanceof MouseEvent) && keyCode!=32 && keyCode!=27 && keyCode!=127 && (keyCode<97 && keyCode>105));
